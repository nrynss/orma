...
- **Numbers are purchasable in the US and Brazil only, and the line rotates.** India-bound calls come from a different number every time, which the API never names. That removes the save-the-contact mitigation rather than weakening it. **Accepted as a risk**, because nothing else is on offer. Onboarding tells the user the call will come from a number they do not recognise and asks them to answer anyway. Watch for India being added.
